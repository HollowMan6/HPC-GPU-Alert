...
Timescale Ingest offers API for data ingestion through HTTP or UDP. We do symmetric encryption on the monitoring data. We derive a cryptographic key from an API key using the PBKDF2-HMAC-SHA256 (Combining Password-Based Key Derivation Function 2, Hash-based Message Authentication Code, and Secure Hash Algorithm 256-bit together), and then use the derived key as the key for AES-GCM (Combining Advanced Encryption Standard and Galois/Counter Mode) encryption. The server is configured to handle incoming data according to the specified protocol. A UDP server is spun up in a separate thread (goroutine in Golang) if UDP is chosen. If HTTP is selected, the server sets up a route to handle incoming POST requests to the \textbf{/write} API endpoint. The transmitted data format follows the InfluxDB line protocol \cite{influxlineprotocol}.
...
\begin{itemize}
    ...
    \item \textbf{Polling with SQL aggregates}: Start/Stop polling as soon as job metadata updates, directly use SQL to do aggregates (without continuous aggregates for caching).
...
We need to benchmark all 4 design choices to reach a final decision, which will be further discussed in Subsection \ref{subsec:performance}.
...
The \textbf{API design} of the Timescale Alert to provide the data for other services is as follows:
...
In addition, the dashboard allows users to disable or re-enable job alerts, so that admins can improve the alert manually and focus more on those jobs, that have not been checked or can be improved.

Aside from sorting the data, the dashboard also allows the viewers to filter the data by job ID, user name, hostname, and alert status, so that viewers can quickly target different entities in question. When we filter by user name or by job ID, the dashboard also allows admins to send emails to the user in question, with one click, from the preset template, for the jobs that are current in alert. One example email generated by the dashboard, when we click the button in Figure \ref{fig_status_dashboard}, is as follows:
...
- GPU 3 on host r15g03 for job 1979, with lifetime average 22.15%, maximum 100%, miminum 0%.
...
2. Find out the history of the job's GPU data by visiting: https://puhti.csc.fi/pun/sys/dashboard/custom/job_monitor
2. Check the job's output and error logs for any error messages.
3. Reduce the number of GPUs requested in your job script.

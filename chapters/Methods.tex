...
Communication between different components during the lifetime of a job is presented in Figure \ref{fig_monitoring}. This subsection will then give a detailed description of the monitoring system design and how they work together.
...
\begin{itemize}
    \item \textbf{CPU usage}: Reads the CPU metrics from \texttt{/proc} for the cores assigned to the job. Since too many cores can be allocated to one job, we can also use the aggregated result of all the assigned cores.
    \item \textbf{Memory usage per job}: Gets the total memory usage from Cgroups for the job per node. This requires that Slurm be set up to use the Cgroups plugin.
...
\end{lstlisting}
    ...
    \item \textbf{start-task}: indicates the start of a specific task within a job. It contains metadata such as the task's process ID, step ID (\textit{-1} indicates no step), node-local task ID for the process within a job (\textit{locaID}), number of processes in the job step or whole heterogeneous job step (\textit{stepTasks}) and loaded modules, which records the modules name as well as their versions separated by a colon. It also includes the GPU energy counter assigned to the job.
...
We use TimescaleDB-specific features to improve query performance. We turn the metrics table into a hypertable with 1-hour chunking. We also create indexes on job IDs, hostnames, and GPU IDs, as those are the keys most commonly used for our SQL group queries. We compress data older than one day and drop data older than one month.
...
The Timescale Reader component facilitates the retrieval of metrics from TimescaleDB for analysis and reporting. It provides RESTful API for building other elements that support administrators in gaining insights into historical resource utilization, aiding in capacity planning, performance optimization, and trend analysis. It also enables the integration of other components to show statistics to users via GUI or CLI. The Timescale Reader complements the real-time monitoring capabilities, providing a comprehensive view of metrics across different time intervals.
...
Below is an example showing the output of the \textit{seff} command. The GPU job efficiency section is printed via the Timescale Reader API.
...
Regarding the design of the alert algorithms for GPU usage, since many high-performance computing applications have load balancing issues regarding pipeline parallelism and the data has highly fluctuated characteristics for multiple GPU jobs, alerting only according to average is neither reliable nor practical, as it may not be fixable easily by the user, so those can be non-critical. Machine learning models can be one way to address the data fluctuation issue by recognizing the data pattern.
...
The alert service we implemented is built on the monitoring system. It communicates with the TimescaleDB through triggers to update the internal state in memory. Whenever we have a new alert or the alert is dismissed, we write the event into logs.

We create or destroy the internal state for specific jobs by listening to job metadata updates generated by the database triggers. We garbage collect stale jobs to avoid memory leaks. We get the aggregation result as soon as data are inserted. For dashboard RESTful API integration, handling the one-writer, multiple-reader problem can be very complex if we directly let the API server access our internal state, as starvation can quickly happen and thus cause significantly reduced performance. So, in the end, we choose to convert the issue into the one-writer, one-reader problem by dumping the alert state with a thread separately at regular intervals into the JSON string so that we can easily tackle the issue by simply using the thread-safe map and mutual exclusion lock.

Since message queues introduce a single point of failure and can be challenging to debug, we decided not to use them. This increases the difficulty of our design, but we do have an alternative: TimescaleDB, which is based on PostgreSQL. PostgreSQL has LISTEN and NOTIFY, so we can use triggers to execute NOTIFY after the insertion of each row, and Timescale Alert can LISTEN to the updates.

We also have continuous aggregates in TimescaleDB \cite{ConAggs}, Spark \cite{10.1145/2783258.2789993}, and Flink \cite{10.14778/3137765.3137777}, which can also be our choice. However, after investigation, we found they do not have sliding-window aggregation support with high-level API. 

Since we want real-time alerts, we want to aggregate with a fixed-size sliding window that considers the latest data and drops the old data at any time, as shown in Figure \ref{fig_sliding_window}. However, TimescaleDB continuous aggregates, Spark, and Flink high-level API can only aggregate with a fixed start time, as shown in Figure \ref{fig_fixed_sliding_window}, which will work badly if we want to check the latest aggregation result at of the clock in this case since we will only have one data to aggregate in that window. Then, it is not very sensible to have an aggregation.
...
Here is an empirical analysis of the design. We can choose either polling or triggers. With polling, the alert delay increases and will likely burden the database heavily with read operations. The alert delay is minimized with triggers, but we might slow down the database for writing operations.

We can also use the continuous aggregates in TimescaleDB. Although the sliding window is not supported, we can overcome this issue by using a weighted average on the last two windows. However, it is still not a good candidate since triggers for continuous aggregates are not supported currently, as confirmed by the issue I opened\footnote{https://github.com/timescale/timescaledb/issues/6500}. So we will have to burden the database by polling, which means many read operations plus many background jobs.

We devised four solutions combining polling triggers and continuous aggregates with and without them.
...
Below is an example showing the GPU alert history output section of the "seff" command. The alert information is printed via the above-mentioned \texttt{/history} endpoint.
...
-------------------------------------------------------------------
GPU Usage
                     Time   Hostname   GPU Id   Value (%)   Normal
